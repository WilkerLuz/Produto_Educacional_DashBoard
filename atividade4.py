...
# DataFrame Usados
df_atividade_4 = pd.read_csv('vpl_atividade_04_com_engajamento.csv')
df_analise_04 = pd.read_csv('analise_todas_atividades_vpl.csv')
df_vpl_04 = pd.read_csv('vpl_01_limpo.csv')

df_analise_04['NotaCP'] = df_analise_04['NotaCP'].str.replace(',','.').astype(float)
df_analise_04['Nota1'] = df_analise_04['Nota1'].str.replace(',','.').astype(float)
df_analise_04['Nota2'] = df_analise_04['Nota2'].str.replace(',','.').astype(float)
df_analise_04['Nota3'] = df_analise_04['Nota3'].str.replace(',','.').astype(float)
df_analise_04['Nota4'] = df_analise_04['Nota4'].str.replace(',','.').astype(float)

df_atividade_4['Media_Notas'] = df_atividade_4['Media_Notas'].str.replace(',','.').astype(float)
...
media_notas = df_atividade_4['Media_Notas'].mean()
desvio_notas = df_atividade_4['Media_Notas'].std()
percentil_25 = np.percentile(df_atividade_4['Media_Notas'], 25)
percentil_50 = np.percentile(df_atividade_4['Media_Notas'], 50)
percentil_75 = np.percentile(df_atividade_4['Media_Notas'], 75)
Media_Engajamento = df_analise_04['engajamento_n4'].mean()
alerta_notas = (df_atividade_4['Media_Notas'] < 7).sum()
alerta_engajamento = (df_analise_04['engajamento_n4'] < 70).sum()
...
heatmap_data_n4 = pd.pivot_table(df_vpl_04, index='NOME', columns='NAME', values='CONTAGEM', aggfunc='mean')

# Crie o heatmap interativo
trace = go.Heatmap(z=heatmap_data_n4.values,
                   x=heatmap_data_n4.columns,
                   y=heatmap_data_n4.index,
                   colorscale='YlGnBu',
...
df_normalized = scaler.fit_transform(df_analise_04[['NotaCP', 'Nota1', 'Nota2', 'Nota3', 'Nota4', 'Experiencia', 'engajamento_n4_media']])

# Executar K-means nos dados normalizados

n_clusters = 3
kmeans = KMeans(n_clusters=n_clusters, n_init=10, random_state=0)
df_analise_04['Cluster'] = kmeans.fit_predict(df_normalized)


# Layout do aplicativo
layout = html.Div([
    html.H1("Atividade 04 - VPL Conceitos Iniciais da Linguagem de Programação C", style=style_h1),

    dcc.Dropdown(
        id='aluno-dropdown_n4',
        options=[
            {'label': aluno, 'value': aluno}
            for aluno in df_atividade_4['NOME_ALUNO']
        ],
        value=df_atividade_4['NOME_ALUNO'][0]
    ),

    dcc.Graph(id='grafico_n4'),
    ...
    dcc.Dropdown(
        id='aluno-dropdown_atraso',
        options=[{'label': str(aluno), 'value': aluno} for aluno in df_vpl_04['NOME'].unique()],
        value=df_vpl_04['NOME'].iloc[0]  # Defina o valor inicial do dropdown
    ),

    # Gráfico para mostrar as questões com atraso
    dcc.Graph(id='grafico-atraso'),
    ...
    dcc.Graph(
        id='grafico_cluster_n4',
        config={'scrollZoom': False},

    ),

    dcc.Dropdown(
        id='cluster-dropdown_n4',
        options=[
    ...
    html.Div([
        html.H3(id='cluster-title_n4'),
        html.Table(id='cluster-list_n4',
                   style={'width': '50%'})
    ]),
...
@callback(
    Output('grafico_n4', 'figure'),
    Input('aluno-dropdown_n4', 'value')
)
def update_graph_boletim(selected_aluno):
    aluno_data = df_atividade_4[df_atividade_4['NOME_ALUNO'] == selected_aluno]
...
def gera_grafico_atraso(selected_aluno):
    aluno_df = df_vpl_04[df_vpl_04['NOME'] == selected_aluno]
    fig = px.bar(aluno_df, x='NAME', color='ATRASO', title=f'Questões com Atraso para Aluno {selected_aluno}')
    return fig

@callback(
    Output('grafico-atraso', 'figure'),
    [Input('aluno-dropdown_atraso', 'value')]
)
...
@callback(
    Output('grafico_cluster_n4', 'figure'),
    Input('cluster-dropdown_n4', 'value')
)

def update_line_chart(selected_cluster):
    df_filtered = df_analise_04[df_analise_04['Cluster'] == selected_cluster]

    fig = go.Figure()

    for index, row in df_filtered.iterrows():
        aluno = row['NOME_ALUNO']
        notas = row[['Nota1', 'Nota2', 'Nota3', 'Nota4']]

        fig.add_trace(go.Scatter(x=['Nota1', 'Nota2', 'Nota3', 'Nota4'],
                                  y=notas,
                                  mode='lines+markers',
                                  name=aluno,
...
@callback(
    [Output('cluster-title_n4', 'children'),
     Output('cluster-list_n4', 'children')],
    Input('cluster-dropdown_n4', 'value')
)
def update_cluster_list(selected_cluster):
    cluster_data = df_analise_04[df_analise_04['Cluster'] == selected_cluster]
