...
# DataFrame Usados
df_atividade_5 = pd.read_csv('vpl_atividade_05_com_engajamento.csv')
df_analise_05 = pd.read_csv('analise_todas_atividades_vpl.csv')
df_vpl_05 = pd.read_csv('vpl_02_limpo.csv')

df_analise_05['NotaCP'] = df_analise_05['NotaCP'].str.replace(',','.').astype(float)
df_analise_05['Nota1'] = df_analise_05['Nota1'].str.replace(',','.').astype(float)
df_analise_05['Nota2'] = df_analise_05['Nota2'].str.replace(',','.').astype(float)
df_analise_05['Nota3'] = df_analise_05['Nota3'].str.replace(',','.').astype(float)
df_analise_05['Nota4'] = df_analise_05['Nota4'].str.replace(',','.').astype(float)

num_questoes = 17

for i in range(1, num_questoes + 1):
    coluna = f'Questão {i}'
    df_atividade_5[coluna] = df_atividade_5[coluna].str.replace(',', '.').astype(float)
...
media_notas = df_atividade_5['Media_Notas'].mean()
desvio_notas = df_atividade_5['Media_Notas'].std()
percentil_25 = np.percentile(df_atividade_5['Media_Notas'], 25)
percentil_50 = np.percentile(df_atividade_5['Media_Notas'], 50)
percentil_75 = np.percentile(df_atividade_5['Media_Notas'], 75)
Media_Engajamento = df_analise_05['engajamento_n5'].mean()
alerta_notas = (df_atividade_5['Media_Notas'] < 7).sum()
alerta_engajamento = (df_analise_05['engajamento_n5'] < 70).sum()
...
heatmap_data_n5 = pd.pivot_table(df_vpl_05, index='NOME', columns='NAME', values='CONTAGEM', aggfunc='mean')

# Crie o heatmap interativo
trace = go.Heatmap(z=heatmap_data_n5.values,
                   x=heatmap_data_n5.columns,
                   y=heatmap_data_n5.index,
                   colorscale='YlGnBu',
...
df_analise_05['NotaCP'].fillna(0, inplace=True)
df_analise_05['Nota1'].fillna(0, inplace=True)
df_analise_05['Nota2'].fillna(0, inplace=True)
df_analise_05['Nota3'].fillna(0, inplace=True)
df_analise_05['Nota4'].fillna(0, inplace=True)
df_analise_05['Nota5'].fillna(0, inplace=True)
df_analise_05['Experiencia'].fillna(0, inplace=True)
df_analise_05['engajamento_n5_media'].fillna(0, inplace=True)

# Normalizar os dados
scaler = StandardScaler()
df_normalized = scaler.fit_transform(df_analise_05[['NotaCP', 'Nota1', 'Nota2', 'Nota3', 'Nota4', 'Nota5', 'Experiencia', 'engajamento_n5_media']])

# Executar K-means nos dados normalizados

n_clusters = 3
kmeans = KMeans(n_clusters=n_clusters, n_init=10, random_state=0)
df_analise_05['Cluster'] = kmeans.fit_predict(df_normalized)

# Layout do aplicativo
layout = html.Div([
    html.H1("Atividade VPL - Estruturas de Decisão da Linguagem de Programação C", style=style_h1),

    dcc.Dropdown(
        id='aluno-dropdown_n5',
        options=[
            {'label': aluno, 'value': aluno}
            for aluno in df_atividade_5['NOME_ALUNO']
        ],
        value=df_atividade_5['NOME_ALUNO'][0]
    ),

    dcc.Graph(id='grafico_n5'),
    ...
    dcc.Dropdown(
        id='aluno-dropdown_atraso_n5',
        options=[{'label': str(aluno), 'value': aluno} for aluno in df_vpl_05['NOME'].unique()],
        value=df_vpl_05['NOME'].iloc[0]  # Defina o valor inicial do dropdown
    ),

    # Gráfico para mostrar as questões com atraso
    dcc.Graph(id='grafico-atraso_n5'),
    ...
    dcc.Graph(
        id='grafico_cluster_n5',
        config={'scrollZoom': False},

    ),

    dcc.Dropdown(
        id='cluster-dropdown_n5',
        options=[
    ...
    html.Div([
        html.H3(id='cluster-title_n5'),
        html.Table(id='cluster-list_n5',
                   style={'width': '50%'})
    ]),
...
@callback(
    Output('grafico_n5', 'figure'),
    Input('aluno-dropdown_n5', 'value')
)
def update_graph_boletim(selected_aluno):
    aluno_data = df_atividade_5[df_atividade_5['NOME_ALUNO'] == selected_aluno]
...
def gera_grafico_atraso(selected_aluno):
    aluno_df = df_vpl_05[df_vpl_05['NOME'] == selected_aluno]
    fig = px.bar(aluno_df, x='NAME', color='ATRASO', title=f'Questões com Atraso para Aluno {selected_aluno}')
    return fig

@callback(
    Output('grafico-atraso_n5', 'figure'),
    [Input('aluno-dropdown_atraso_n5', 'value')]
)
...
@callback(
    Output('grafico_cluster_n5', 'figure'),
    Input('cluster-dropdown_n5', 'value')
)

def update_line_chart(selected_cluster):
    df_filtered = df_analise_05[df_analise_05['Cluster'] == selected_cluster]

    fig = go.Figure()

    for index, row in df_filtered.iterrows():
        aluno = row['NOME_ALUNO']
        notas = row[['Nota1', 'Nota2', 'Nota3', 'Nota4', 'Nota5']]

        fig.add_trace(go.Scatter(x=['Nota1', 'Nota2', 'Nota3', 'Nota4', 'Nota5'],
                                  y=notas,
                                  mode='lines+markers',
                                  name=aluno,
...
@callback(
    [Output('cluster-title_n5', 'children'),
     Output('cluster-list_n5', 'children')],
    Input('cluster-dropdown_n5', 'value')
)
def update_cluster_list(selected_cluster):
    cluster_data = df_analise_05[df_analise_05['Cluster'] == selected_cluster]
